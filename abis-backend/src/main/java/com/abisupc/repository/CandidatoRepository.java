...
public class CandidatoRepository implements Repository<Candidato> {
    ...
    /**
     * Inserta los datos básicos del candidato y obtiene de forma segura el consecutivo de la secuencia.
     *
     * <p>Asegura la atomicidad del identificador consultando la pseudo-columna {@code seq_candidatos.CURRVAL}
     * sobre la tabla del sistema {@code DUAL} inmediatamente después de haber ejecutado la sentencia de inserción.
     *
     * @param c entidad de negocio candidato a asentar
     * @return identificador numérico único (PK) generado por Oracle para el nuevo registro
     * @throws RuntimeException si el motor relacional no logra resolver el valor actual de la secuencia
     */
    ...
    /**
     * Inscribe formalmente a un candidato previamente registrado en una contienda electoral determinada.
     *
     * <p>Aplica una restricción estricta de negocio validando que el número identificador de campaña
     * (número en el tarjetón) esté completamente disponible para la elección parametrizada.
     *
     * @param idCandidato identificador del candidato a postular
     * @param idEleccion identificador de la elección donde participará
     * @param numeroCampania número asignado en el tarjetón electoral
     * @param cargo denominación oficial del puesto gubernamental o corporativo al que aspira
     * @throws IllegalArgumentException si el número de campaña ya está ocupado por otro participante
     * @throws RuntimeException si se produce una falla de integridad o clave duplicada
     */
    ...
    /**
     * Mapea una fila del ResultSet general de candidatos a su correspondiente POJO de dominio.
     */
    ...
    /**
     * Mapea una fila del ResultSet de escrutinio compuesto hacia la entidad relacional extendida.
     */
    ...
    /**
     * Mecanismo de migración automática del diccionario de datos de Oracle.
     *
     * <p>Inspecciona la tabla del sistema {@code USER_TAB_COLUMNS} para verificar la presencia del
     * atributo {@code FOTO_URL}. Si determina la ausencia de la columna, ejecuta de manera segura
     * una instrucción de definición de datos {@code ALTER TABLE} para extender la estructura física.
     *
     * @param conn conexión transaccional activa con la base de datos
     * @throws SQLException si falla el escaneo del diccionario de datos o la ejecución del DDL
     */
